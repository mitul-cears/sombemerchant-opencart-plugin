...
class ControllerExtensionPaymentSombeMerchant extends Controller
{
	...
	public function checkout()
    {
        ...
        $sombe_order = \SombeMerchant\Merchant\Order::create(array(
            ...
            'callback_url' => html_entity_decode($this->url->link('extension/payment/sombemerchant/callback', array('sombe_token' => $token), true)),
            'success_url' => html_entity_decode($this->url->link('extension/payment/sombemerchant/success', array('sombe_token' => $token), true)),
        ...
        if ($sombe_order) {
            ...
            $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get('payment_sombemerchant_order_status_id'));
    ...
    public function success()
    {
        ...
        if (empty($order) || strcmp($order['token'], $this->request->get['sombe_token']) !== 0) {
    ...
    public function callback()
    {
        $this->load->model('checkout/order');
        $this->load->model('extension/payment/sombemerchant');
        $post_data = \SombeMerchant\SombeMerchant::decrypt($this->config->get('payment_sombemerchant_api_secret'),$this->request->post['encData']);
        ...
        if (!empty($order_info) && !empty($ext_order) && strcmp($ext_order['token'], $post_data['token']) === 0) {
            ...
            if ($sombe_order) {
                switch ($sombe_order->status) {
                    case 'Completed':
                        $sombe_order_status = 'payment_sombemerchant_paid_status_id';
                        break;
                    case 'Invalid':
                        $sombe_order_status = 'payment_sombemerchant_invalid_status_id';
                        break;
                    case 'Expired':
                        $sombe_order_status = 'payment_sombemerchant_expired_status_id';
                        break;
                    case 'Canceled':
                        $sombe_order_status = 'payment_sombemerchant_canceled_status_id';
                        break;
                    case 'Refunded':
                        $sombe_order_status = 'payment_sombemerchant_refunded_status_id';
                        break;
                    default:
    ...
    private function setupSombeMerchantClient()
    {
        \SombeMerchant\SombeMerchant::config(array(
            'auth_token' => empty($this->config->get('payment_sombemerchant_api_auth_token')) ? $this->config->get('payment_sombemerchant_api_secret') : $this->config->get('payment_sombemerchant_api_auth_token'),
            'user_agent' => 'SombeMerchant - OpenCart v' . VERSION . ' Extension v' . SOMBEMERCHANT_OPENCART_EXTENSION_VERSION
        ));
    }
}